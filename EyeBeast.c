...
#define EVENT_SPEED_COOLDOWN 5
#define EVENTS_COOLDOWN_MESSAGE 2
#define NUMBER_EVENTS 5
#define DEFAULT_HERO_LIFES 1;

const char *randomScenarios[NUMBER_EVENTS] = {"KILL 1 ENEMY", "ADDED 1 MORE ENEMY", "1 MORE LIFE", "ENEMIES FAST", "ENEMIES SLOW"};
...
void heroAnimation(Game g, Actor a) {
    ...
    if (cellIsEmpty(g, nx, ny)) {
        ...
        if (g->world[nx][ny]->kind == CHERRY) {
            executeCherryOptions(g, tyRand(NUMBER_EVENTS+1));
...
void initVariables(Game g){
    g->cherryPlaced = false;
    g->heroLifes = DEFAULT_HERO_LIFES;
    g->monsterSpeed = DEFAULT_SPEED;
    g->lastActionSpeed = 0;
}
...
void runAllMonstersAnimation(Game g) {
...
void gameAnimation(Game g) {
    ...
    if( g->monsterSpeed != DEFAULT_SPEED) {
        if(g->lastActionSpeed + EVENT_SPEED_COOLDOWN >= tySeconds())
                runAllMonstersAnimation(g);
         else
            g->monsterSpeed = DEFAULT_SPEED;
    }else{
            runAllMonstersAnimation(g);
...
void status(Game game) {
    ...
    if (game->lastActionTime + EVENTS_COOLDOWN_MESSAGE >= tySeconds() || game->lastActionSpeed + EVENT_SPEED_COOLDOWN >= tySeconds())
